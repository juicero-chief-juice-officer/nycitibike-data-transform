...
resource "google_bigquery_table" "external_tables" {
  ...
  external_data_configuration {
    ...
    compression   = "GZIP"
    source_uris   = ["gs://${each.value.gcs_bucket_name}/${each.value.gcs_path}*.parquet"]
  }
  time_partitioning {
    type  = "DAY"
...
data "google_iam_policy" "p4sa-secretAccessor" {
  ...
  binding {
    ...
    members = ["serviceAccount:service-${output.project_number}@gcp-sa-cloudbuild.iam.gserviceaccount.com"]
  }
}
