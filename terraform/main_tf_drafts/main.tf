...
resource "null_resource" "stall_30_seconds" {
  triggers = {
    key_build = google_service_account_key.key_clrn.private_key
  }
...
resource "google_secret_manager_secret_version" "clrn_secret_version" {
  ...
  secret_data = google_service_account_key.key_clrn.private_key
  depends_on  = [null_resource.stall_30_seconds, google_secret_manager_secret.clrn_secret, google_service_account_key.key_clrn]
}
